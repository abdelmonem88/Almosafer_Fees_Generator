...
function calculator() {
	...
	let adtsVat = parseFloat(totalAdt) * 0.05;
	...
	if (mosafer.value == '' || taxes.value == '' || adtPenalty.value == '' || adtNumber.value == '') {
		...
		if (totalVat != 0) {
			uiArVat.innerHTML = `ضريبة القيمة المضافة = ${totalVat} ${chgArCurrency.value}`;
		}
		if (fixedToatl != 0) {
		...
		if (totalVat != 0) {
			uiEnVat.innerHTML = `Total VAT = ${totalVat} ${chgEnCurrency.value}`;
		}
		if (fixedToatl != 0) {
